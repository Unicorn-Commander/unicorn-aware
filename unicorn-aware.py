...
try:
    from config import get_config, get_ui_config, get_audio_config, get_processing_config
...
try:
    from logging_config import setup_logging, get_logger, LoggingSession
    LOGGING_AVAILABLE = True
    print("✅ Logging system available")
except ImportError as e:
    LOGGING_AVAILABLE = False
    print(f"⚠️ Logging system not available: {e}")
    print("📝 Using print statements for logging")

# Add project path
sys.path.insert(0, '/home/ucadmin/Development/whisper_npu_project')

# Import topical filtering framework with better error handling
try:
    ...
    print("✅ Topical Filtering Framework available")
except ImportError as e:
    FILTERING_AVAILABLE = False
    print(f"⚠️ Topical Filtering Framework not available: {e}")
    print("📝 Some conversation analysis features will be limited")

# Import enhanced topical filtering with fallback
try:
    ...
    print("📝 Advanced emotion and complaint detection will be disabled")
    
    # Create fallback classes
    class FallbackFilter:
        def analyze(self, text):
            return {"text": text, "confidence": 0.0, "fallback": True}
    
    class EnhancedTopicalFilterManager:
        def __init__(self):
            self.emotional_filter = FallbackFilter()
            self.complaint_filter = FallbackFilter()
        
        def analyze_all(self, text):
            return {
                "emotion": {"text": text, "confidence": 0.0, "fallback": True},
                "complaint": {"text": text, "confidence": 0.0, "fallback": True}
            }
    
    EmotionalRecognitionFilter = FallbackFilter
    ComplaintDetectionFilter = FallbackFilter

# Qt6/PySide6 imports with comprehensive fallback
try:
    ...
    try:
        ...
        QT6_AVAILABLE = True
        QT_FRAMEWORK = "PyQt6"
        print("✅ Using PyQt6 as fallback")
    except ImportError as e2:
        print(f"❌ PyQt6 also not available: {e2}")
        print("📝 Install Qt6 with: pip install PySide6 or pip install PyQt6")
        print("💡 GUI cannot start without a Qt6 framework")
        sys.exit(1)
...
try:
    ...
    print("📝 Rich help content will use fallback display")
...
try:
    ...
    ADVANCED_NPU_AVAILABLE = True
    print("✅ Advanced NPU Backend available")
except ImportError as e:
    ADVANCED_NPU_AVAILABLE = False
...
try:
    from tts_integration.simple_tts import SimpleTTS, is_tts_available
    TTS_AVAILABLE = is_tts_available()
    if TTS_AVAILABLE:
        print("✅ Simple TTS available")
    else:
        print("⚠️ TTS available but not functional (fallback mode)")
except ImportError as e:
    TTS_AVAILABLE = False
...
class ModelManager:
    ...
    def _load_model(self, model_type: str, model_name: str = None):
        ...
        try:
            if model_type == "whisper_npu":
                return self._load_whisper_npu(model_name)
            elif model_type == "advanced_npu":
                return self._load_advanced_npu(model_name)
            elif model_type == "igpu":
                return self._load_igpu_backend(model_name)
            elif model_type == "tts":
                return self._load_tts_engine()
            else:
                return None
    ...
    def _load_whisper_npu(self, model_name):
        """Load WhisperX NPU model"""
        if not NPU_AVAILABLE:
            return None
        try:
            from always_listening_npu import AlwaysListeningNPU
            return AlwaysListeningNPU()
        except Exception as e:
            print(f"⚠️ Failed to load WhisperX NPU: {e}")
            return None
    
    def _load_advanced_npu(self, model_name):
        """Load Advanced NPU backend"""
        if not ADVANCED_NPU_AVAILABLE:
            return None
        try:
            from advanced_npu_backend import AdvancedNPUBackend
            return AdvancedNPUBackend()
        except Exception as e:
            print(f"⚠️ Failed to load Advanced NPU: {e}")
            return None
    
    def _load_igpu_backend(self, model_name):
        """Load iGPU backend"""
        if not IGPU_BACKEND_AVAILABLE:
            return None
        try:
            from igpu_backend import IGPUBackend
            return IGPUBackend()
        except Exception as e:
            print(f"⚠️ Failed to load iGPU backend: {e}")
            return None
    ...
    def _load_tts_engine(self):
        ...
        if not TTS_AVAILABLE:
            return None
        try:
            from tts_integration.kokoro_mlir_integration import KokoroTTS
            return KokoroTTS()
        except Exception as e:
            print(f"⚠️ Failed to load TTS engine: {e}")
            return None
    
    def is_model_ready(self, model_type: str, model_name: str = None) -> bool:
        ...
        for key, model in self._models.items():
            if hasattr(model, 'cleanup'):
                try:
                    ...
                    print(f"⚠️ Error cleaning up {key}: {e}")
        self._models.clear()
...
class AlwaysListeningGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        if LOGGING_AVAILABLE:
            try:
                ...
                self.logger.info("🦄 Unicorn Aware GUI Starting")
                self.logger.log_system_info()
                
                # Log module availability
                modules = {
                    "PySide6/PyQt6": QT6_AVAILABLE,
                    "WebEngine": WEBENGINE_AVAILABLE,
                    "NPU Backend": NPU_AVAILABLE,
                    "Advanced NPU": ADVANCED_NPU_AVAILABLE,
                    "iGPU Backend": IGPU_BACKEND_AVAILABLE,
                    "TTS": TTS_AVAILABLE,
                    "Enhanced Filtering": ENHANCED_FILTERING_AVAILABLE,
                    "Topical Filtering": FILTERING_AVAILABLE,
                    "Configuration": CONFIG_AVAILABLE
                }
                self.logger.log_module_status(modules)
        ...
        if CONFIG_AVAILABLE:
            ...
            self.tooltips_enabled = self.preferences.get("enable_tooltips", ui_config.get("enable_tooltips", True))
            self.resize(self.preferences.get("window_width", ui_config.get("window_width", 1200)), self.preferences.get("window_height", ui_config.get("window_height", 800)))
            
            # Apply processing configuration
            self.max_results_history = processing_config.get("max_transcription_length", 1000)
            
            if self.logger:
                self.logger.info(f"✅ Configuration loaded - UI: {ui_config.get('theme', 'default')} theme")
            else:
                print(f"✅ Configuration loaded - UI: {ui_config.get('theme', 'default')} theme")
        else:
            # Fallback to hardcoded defaults
            self.tooltips_enabled = self.preferences.get("enable_tooltips", True)
            self.resize(self.preferences.get("window_width", 1200), self.preferences.get("window_height", 800))
            self.max_results_history = 1000
        
        # Settings
        self.download_progress = {}
        self.download_cancelled = False
        self.tts_integration_dir = os.path.join(os.path.dirname(__file__), "tts_integration")
        ...
        self.log_info("🚀 GUI initialization complete")

    def closeEvent(self, event):
        """Handle application close event to save preferences."""
        self.log_info("Application closing. Saving preferences...")
    ...
    def log_debug(self, message: str):
        """Log debug message with fallback to print"""
        if self.logger:
    ...
    def setup_keyboard_shortcuts(self):
        ...
        # Import QKeySequence and QShortcut
        from PySide6.QtGui import QKeySequence, QShortcut
        
        # System control shortcuts
        self.shortcut_init = QShortcut(QKeySequence("Ctrl+I"), self)
        self.shortcut_init.activated.connect(self.initialize_always_listening)
        
        self.shortcut_start = QShortcut(QKeySequence("Ctrl+S"), self)
        self.shortcut_start.activated.connect(self.start_always_listening)
        
        self.shortcut_stop = QShortcut(QKeySequence("Ctrl+Shift+S"), self)
        self.shortcut_stop.activated.connect(self.stop_always_listening)
        
        self.shortcut_shutdown = QShortcut(QKeySequence("Ctrl+Q"), self)
        self.shortcut_shutdown.activated.connect(self.shutdown_system)
        
        # UI shortcuts
        self.shortcut_clear = QShortcut(QKeySequence("Ctrl+L"), self)
        self.shortcut_clear.activated.connect(self.clear_transcription_results)
        
        self.shortcut_help = QShortcut(QKeySequence("F1"), self)
        self.shortcut_help.activated.connect(self.open_help_window)
        ...
        self.shortcut_tab3.activated.connect(lambda: self.tab_widget.setCurrentIndex(2))
        
        # Status display shortcut
        self.shortcut_status = QShortcut(QKeySequence("Ctrl+D"), self)
        self.shortcut_status.activated.connect(lambda: self.update_status_display("⚡ Status: System running, shortcuts active!"))
    
    def log_exception(self, message: str, exception: Exception):
        """Log exception with fallback to print"""
        if self.logger:
            self.logger.log_error_with_context(exception, {"message": message})
        else:
    ...
    def create_always_listening_tab(self):
        ...
        self.init_button.setStyleSheet("""
            ...
            QPushButton:hover {
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                    stop:0 #5ba0f2, stop:1 #4680d2);
            }
            QPushButton:pressed {
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                    stop:0 #3a80d2, stop:1 #2970ad);
            }
            QPushButton:disabled {
                background: #555;
                color: #888;
        ...
        # Start button with success theme
        ...
        # Stop button with warning theme
        ...
        # Shutdown button with danger theme
        ...
        # Enhanced status indicators with modern cards
        ...
        self.recording_status = QLabel("🎙️ Recording: Not Recording")
        self.processing_status = QLabel("🧠 Processing: Idle")
        
        # Modern card style for inactive state
        inactive_style = """
            QLabel {
                font-size: 14px;
                font-weight: bold;
        ...
        self.speak_button = QPushButton("🗣️ Speak Last")
        self.speak_button.clicked.connect(self.speak_last_transcription)
        results_controls.addWidget(self.speak_button)

        self.auto_speak_checkbox = QCheckBox("Auto Speak")
        self.auto_speak_checkbox.setChecked(True)
        results_controls.addWidget(self.auto_speak_checkbox)
...
- Export options for TXT and JSON formats""")
    ...
    def create_model_management_tab(self):
        ...
        self.download_type.addItems([
            "ONNX Whisper", 
            "OpenAI Whisper", 
            "Kokoro TTS", 
            "Quantized NPU Models",
            "Custom"
        ])
        ...
        download_selection.addWidget(QLabel("Repository Browser:"), 3, 0)
        self.repo_browser_combo = QComboBox()
        self.repo_browser_combo.addItems([
            "Browse Unicorn-Commander/npu-prebuilds",
            "Browse onnx-community models", 
            "Browse microsoft/whisper models"
        ])
        download_selection.addWidget(self.repo_browser_combo, 3, 1)
        ...
        self.browse_repo_button = QPushButton("🔍 Browse Repository")
        self.browse_repo_button.clicked.connect(self.browse_repository)
        download_controls.addWidget(self.browse_repo_button)
        
        self.import_button = QPushButton("📂 Import Local Model")
        self.import_button.clicked.connect(self.import_local_model)
        download_controls.addWidget(self.import_button)
    ...
    def download_model(self):
        ...
        model_size = self.download_size.currentText().split(" ")[0]
        custom_url = self.custom_url.text()

        if model_type == "Custom" and custom_url:
            self.download_file(custom_url, self.tts_integration_dir if "kokoro" in custom_url.lower() else "./models")
        else:
            if model_type == "Kokoro TTS":
                model_url = f"https://huggingface.co/Unicorn-Commander/npu-prebuilds/resolve/main/kokoro-v1.0.onnx"
                voices_url = f"https://huggingface.co/Unicorn-Commander/npu-prebuilds/resolve/main/voices-v1.0.bin"
                self.download_file(model_url, self.tts_integration_dir)
                self.download_file(voices_url, self.tts_integration_dir)
            elif model_type == "Quantized NPU Models":
                # Download quantized models - try multiple sources for best availability
                model_files = {
                    "tiny": ["whisper-tiny-q8.onnx", "whisper-tiny-encoder-q8.onnx"],
                    "base": ["whisper-base-q8.onnx", "whisper-base-encoder-q8.onnx"],
                    "small": ["whisper-small-q8.onnx", "whisper-small-encoder-q8.onnx"],
                    "medium": ["whisper-medium-q8.onnx", "whisper-medium-encoder-q8.onnx"],
                    "large": ["whisper-large-q8.onnx", "whisper-large-encoder-q8.onnx"],
                    "large-v2": ["whisper-large-v2-q8.onnx", "whisper-large-v2-encoder-q8.onnx"],
                    "large-v3": ["whisper-large-v3-q8.onnx", "whisper-large-v3-encoder-q8.onnx"]
                }
                
                # Try multiple download sources
                download_sources = [
                    "https://huggingface.co/Unicorn-Commander/npu-prebuilds/resolve/main/",
                    "https://github.com/unicorn-commander/npu-models/releases/download/v1.0/",
                    "https://raw.githubusercontent.com/unicorn-commander/npu-models/main/quantized/"
                ]
                
                if model_size in model_files:
                    download_dir = f"./models/quantized/whisper-{model_size}-q8"
                    for file_name in model_files[model_size]:
                        # Try each download source until one works
                        download_successful = False
                        for source_url in download_sources:
                            try:
                                model_url = source_url + file_name
                                self.download_file(model_url, download_dir)
                                self.log_info(f"Downloading quantized model: {file_name} from {source_url}")
                                download_successful = True
                                break
                            except Exception as e:
                                self.log_debug(f"Failed to download from {source_url}: {e}")
                                continue
                        
                        if not download_successful:
                            self.update_status_display(f"⚠️ Could not download {file_name} from any source")
                else:
                    self.update_status_display(f"⚠️ Quantized model size '{model_size}' not available")
            elif model_type == "ONNX Whisper":
                model_url = f"https://huggingface.co/onnx-community/whisper-{model_size}/resolve/main/model.onnx"
                self.download_file(model_url, f"./models/whisper-{model_size}")
            elif model_type == "OpenAI Whisper":
                self.update_status_display("Downloading OpenAI Whisper models is handled by the whisper library.")

    def download_file(self, url, download_dir):
        """Download a file from a URL to a directory with real implementation."""
        import threading
        ...
        def download_worker():
            try:
                ...
                if not file_name or file_name == "main":
                    file_name = f"model_{int(time.time())}.onnx"
                    
                download_path = os.path.join(download_dir, file_name)

                if os.path.exists(download_path):
                    self.update_status_display(f"✅ {file_name} already exists.")
                    return

                self.update_status_display(f"🔄 Downloading {file_name}...")
                ...
                try:
                        ...
                        for chunk in response.iter_content(chunk_size=8192):
                            if chunk:
                                ...
                                if total_size > 0:
                                    ...
                                    self.update_status_display(f"📥 Downloading {file_name}... {progress}% ({mb_downloaded:.1f}/{mb_total:.1f} MB)")
                    
                    self.update_status_display(f"✅ {file_name} downloaded successfully ({downloaded_size:,} bytes)")
                    ...
                    self.update_status_display(f"✅ {file_name} downloaded successfully ({file_size:,} bytes)")

            except Exception as e:
                self.log_error(f"Download failed for {url}: {e}")
                self.update_status_display(f"❌ Download failed for {file_name}: {e}")
        
        # Run download in background thread
        threading.Thread(target=download_worker, daemon=True).start()
    
    def browse_repository(self):
        """Browse and display available models from selected repository"""
        repo_name = self.repo_browser_combo.currentText()
        
        if "Unicorn-Commander/npu-prebuilds" in repo_name:
            self.show_available_models("Unicorn-Commander/npu-prebuilds")
        elif "onnx-community" in repo_name:
            self.show_available_models("onnx-community")
        elif "microsoft/whisper" in repo_name:
            self.show_available_models("microsoft/whisper")
    
    def show_available_models(self, repo_base):
        """Show available models from a repository"""
        try:
            if repo_base == "Unicorn-Commander/npu-prebuilds":
                # Show quantized models available
                models = [
                    "🎯 whisper-tiny-q8.onnx (Quantized, NPU optimized)",
                    "🎯 whisper-base-q8.onnx (Quantized, NPU optimized)", 
                    "🎯 whisper-small-q8.onnx (Quantized, NPU optimized)",
                    "🎯 whisper-medium-q8.onnx (Quantized, NPU optimized)",
                    "🎯 whisper-large-q8.onnx (Quantized, NPU optimized)",
                    "🎯 whisper-large-v2-q8.onnx (Quantized, NPU optimized)",
                    "🎯 whisper-large-v3-q8.onnx (Quantized, NPU optimized)",
                    "🔊 kokoro-v1.0.onnx (TTS Engine)",
                    "🎵 voices-v1.0.bin (TTS Voices)"
                ]
                info_text = "✨ **Quantized NPU Models** - Optimized for AMD NPU hardware with 8-bit quantization for faster inference and lower memory usage."
            
            elif repo_base == "onnx-community":
                models = [
                    "🌟 whisper-tiny (ONNX format, 39MB)",
                    "🌟 whisper-base (ONNX format, 74MB)",
                    "🌟 whisper-small (ONNX format, 244MB)",
                    "🌟 whisper-medium (ONNX format, 769MB)",
                    "🌟 whisper-large (ONNX format, 1550MB)",
                    "🌟 whisper-large-v2 (ONNX format, 1550MB)",
                    "🌟 whisper-large-v3 (ONNX format, 1550MB)"
                ]
                info_text = "🔧 **ONNX Community Models** - Standard ONNX format models compatible with various inference engines."
            
            else:  # microsoft/whisper
                models = [
                    "🏢 whisper-tiny (PyTorch format)",
                    "🏢 whisper-base (PyTorch format)", 
                    "🏢 whisper-small (PyTorch format)",
                    "🏢 whisper-medium (PyTorch format)",
                    "🏢 whisper-large (PyTorch format)",
                    "🏢 whisper-large-v2 (PyTorch format)",
                    "🏢 whisper-large-v3 (PyTorch format)"
                ]
                info_text = "🏢 **Microsoft Official Models** - Original PyTorch models from Microsoft/OpenAI."
            
            # Create dialog to show models
            from PySide6.QtWidgets import QDialog, QVBoxLayout, QTextEdit, QDialogButtonBox
            
            dialog = QDialog(self)
            dialog.setWindowTitle(f"Available Models - {repo_base}")
            ...
            content = f"<h3>📦 {repo_base}</h3>\n"
            content += f"<p>{info_text}</p>\n"
            content += "<h4>Available Models:</h4>\n<ul>\n"
            
            for model in models:
                content += f"<li>{model}</li>\n"
            
            content += "</ul>\n"
            content += "<p><strong>Usage:</strong> Select model type and size in the dropdown above, then click Download Model.</p>"
            ...
            self.log_error(f"Error browsing repository: {e}")
            self.update_status_display(f"❌ Error browsing repository: {e}")
    ...
    def import_local_model(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getOpenFileName(
                ...
                "Import Model File",
                "",
                "Model Files (*.onnx *.pt *.pth *.bin);;All Files (*)"
            )
            ...
            if file_path:
                ...
                if file_ext == '.onnx':
                    if 'whisper' in file_name.lower():
                        dest_dir = "./models/imported/whisper"
                    elif 'kokoro' in file_name.lower() or 'tts' in file_name.lower():
                        dest_dir = "./tts_integration/imported"
                    else:
                        dest_dir = "./models/imported/other"
                elif file_ext in ['.pt', '.pth']:
                    dest_dir = "./models/imported/pytorch"
                elif file_ext == '.bin':
                    dest_dir = "./models/imported/binary"
                else:
                    dest_dir = "./models/imported/other"
                ...
                # Copy file
                self.update_status_display(f"📂 Importing {file_name}...")
                shutil.copy2(file_path, dest_path)
                
                file_size = os.path.getsize(dest_path)
                self.update_status_display(f"✅ Model imported successfully: {file_name} ({file_size:,} bytes)")
                self.log_info(f"Model imported: {file_name} -> {dest_path}")
                
        except Exception as e:
            self.log_error(f"Error importing model: {e}")
            self.update_status_display(f"❌ Error importing model: {e}")
    ...
    def create_kokoro_tts_tab(self):
        """Create the Kokoro TTS integration tab."""
        tab = QWidget()
        layout = QVBoxLayout(tab)

        # Title
        title = QLabel("🔊 Kokoro TTS Synthesis")
        title.setStyleSheet("""
            font-size: 28px; 
            font-weight: 900; 
            color: #61dafb; 
        ...
        if TTS_AVAILABLE and self.tts_engine:
            self.tts_voice_combo.addItems(self.tts_engine.get_voices())
        tts_layout.addWidget(self.tts_voice_combo, 0, 1)

        tts_layout.addWidget(QLabel("Text to Synthesize:"), 1, 0)
        self.tts_text_input = QLineEdit("Hello, world! This is a test of the Kokoro TTS engine.")
        tts_layout.addWidget(self.tts_text_input, 1, 1)

        self.tts_synthesize_button = QPushButton("▶️ Synthesize and Play")
        self.tts_synthesize_button.clicked.connect(self.synthesize_and_play_tts)
        tts_layout.addWidget(self.tts_synthesize_button, 2, 0, 1, 2)

        self.tts_stop_button = QPushButton("⏹️ Stop Playback")
        self.tts_stop_button.clicked.connect(self.stop_tts_playback)
        tts_layout.addWidget(self.tts_stop_button, 3, 0, 1, 2)

        layout.addWidget(tts_group)

        # TTS Status
        status_group = QGroupBox("TTS Status")
        status_layout = QVBoxLayout(status_group)
        self.tts_status_label = QLabel("TTS Engine: Not Initialized")
        status_layout.addWidget(self.tts_status_label)
        layout.addWidget(status_group)

        self.tab_widget.addTab(tab, "🔊 Kokoro TTS")

    def synthesize_and_play_tts(self):
        if not TTS_AVAILABLE or not self.tts_engine:
            self.update_status_display("⚠️ TTS not available or initialized.")
            return
        ...
        if not text.strip():
            self.update_status_display("⚠️ Cannot synthesize empty text.")
            return
    ...
    def stop_tts_playback(self):
        ...
        # The current TTS implementation plays audio in a separate thread and doesn't have a direct stop mechanism.
        self.update_status_display("⚠️ Stop playback is not yet implemented.")
        ...
        try:
            ...
            def init_thread():
                try:
                    ...
                    use_advanced = ADVANCED_NPU_AVAILABLE and not whisper_model.startswith("📦")
                    ...
                    if success:
                        ...
                        if TTS_AVAILABLE:
                            self.update_status_display("🔊 TTS engine will be loaded when needed (lazy loading)")
                            # TTS engine will be loaded lazily when first used
            ...
            # Disable init button with visual feedback
    ...
    def on_initialization_complete(self, success: bool):
        ...
        if success:
            ...
            if TTS_AVAILABLE and self.tts_engine:
                self.tts_voice_combo.addItems(self.tts_engine.get_voices())
                self.tts_status_label.setText("TTS Engine: Ready")
        else:
    ...
    def speak_text(self, text):
        if not TTS_AVAILABLE:
            self.update_status_display("⚠️ TTS not available.")
            return

        def speak():
            try:
                self.update_status_display(f'Synthesizing: "{text[:30]}..."')
                
                # Lazy load TTS engine
                if not self.tts_engine:
                    self.update_status_display("🔊 Loading TTS engine...")
                    self.tts_engine = self.model_manager.get_model("tts")
                    if not self.tts_engine:
                        self.update_status_display("❌ Failed to load TTS engine")
                        return
                
                self.tts_engine.synthesize_and_play(text)
                self.update_status_display("✅ Speech finished.")
            except Exception as e:
                self.update_status_display(f"❌ TTS error: {e}")

        threading.Thread(target=speak, daemon=True).start()

    def speak_last_transcription(self):
        if self.current_results:
            last_result = self.current_results[-1]
            text_to_speak = last_result.get('text', '')
            if text_to_speak:
                self.speak_text(text_to_speak)
            else:
                self.update_status_display("⚠️ No text in last transcription to speak.")
        else:
            self.update_status_display("⚠️ No transcriptions available to speak.")
    ...
    def on_transcription_result(self, result: Dict[str, Any]):
        ...
        try:
            ...
            if self.auto_speak_checkbox.isChecked():
                self.speak_text(original_text)
    ...
    def update_status_indicators(self):
        ...
        if self.is_always_listening:
            ...
            if activation_mode in ["wake_word", "hybrid"]:
                ...
                self.wake_word_status.setStyleSheet("""
                    QLabel {
                        font-size: 14px;
                        font-weight: bold;
                        padding: 12px;
            ...
            self.recording_status.setText("🎙️ Recording: Not Recording")
    ...
    def reset_status_indicators(self):
        ...
        self.vad_status.setStyleSheet(inactive_style)
        
        self.wake_word_status.setText("👂 Wake Word: Inactive")
        self.wake_word_status.setStyleSheet(inactive_style)
        
        self.recording_status.setText("🎙️ Recording: Not Recording")
        self.recording_status.setStyleSheet(inactive_style)
        
        self.processing_status.setText("🧠 Processing: Idle")
        self.processing_status.setStyleSheet(inactive_style)
...
"""
    ...
    def toggle_tooltips(self, enabled: bool):
        ...
        self.preferences.set("enable_tooltips", enabled)
        self.setup_tooltips()
        self.update_status_display(f"💡 Tooltips {'enabled' if enabled else 'disabled'}")
        ...
        try:
            ...
            def download_thread():
                try:
                    ...
                    elif model_type == "Kokoro TTS":
                        # For now, we only have one Kokoro model
                        model_url = "https://huggingface.co/Unicorn-Commander/npu-prebuilds/resolve/main/kokoro-v1.0.onnx"
                        voices_url = "https://huggingface.co/Unicorn-Commander/npu-prebuilds/resolve/main/voices-v1.0.bin"
                        self.download_file(model_url, self.tts_integration_dir)
                        self.download_file(voices_url, self.tts_integration_dir)
    ...
    def apply_settings(self):
        ...
        try:
            ...
            self.preferences.set("enable_tooltips", self.tooltips_enabled) # Save tooltip preference
            self.toggle_tooltips(self.tooltips_enabled)
            
            # Save NPU priority
            npu_priority = self.npu_priority.currentText()
            self.preferences.set("npu_priority", npu_priority)

            # Save window size
            self.preferences.set("window_width", self.width())
            self.preferences.set("window_height", self.height())

            # Create transcript directory if it doesn't exist
            transcript_dir = Path(self.transcript_location.text())
            transcript_dir.mkdir(parents=True, exist_ok=True)
            self.preferences.set("transcript_location", str(transcript_dir)) # Save transcript location
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Unicorn Aware - NPU Speech Recognition System')
    parser.add_argument('--version', action='version', version='Unicorn Aware 2.0-Qt6')
    parser.add_argument('--cli', action='store_true', help='Run in CLI mode (no GUI)')
    parser.add_argument('--process', metavar='FILE', help='Process audio file and exit')
    parser.add_argument('--model', default='base', help='Model to use (default: base)')
    parser.add_argument('--engine', default='igpu', choices=['igpu', 'npu', 'advanced', 'cpu'], 
                        help='Processing engine (default: igpu)')
    parser.add_argument('--output', metavar='FILE', help='Output file for transcription')
    ...
    if args.config:
        print("📊 Unicorn Aware Configuration Status")
        print("=" * 40)
        print(f"Configuration System: {'✅ Available' if CONFIG_AVAILABLE else '❌ Not Available'}")
        print(f"Logging System: {'✅ Available' if LOGGING_AVAILABLE else '❌ Not Available'}")
        print(f"NPU Backend: {'✅ Available' if NPU_AVAILABLE else '❌ Not Available'}")
        print(f"Advanced NPU: {'✅ Available' if ADVANCED_NPU_AVAILABLE else '❌ Not Available'}")
        print(f"iGPU Backend: {'✅ Available' if IGPU_BACKEND_AVAILABLE else '❌ Not Available'}")
        print(f"TTS System: {'✅ Available' if TTS_AVAILABLE else '❌ Not Available'}")
        print(f"Enhanced Filtering: {'✅ Available' if ENHANCED_FILTERING_AVAILABLE else '❌ Not Available'}")
        print(f"Topical Filtering: {'✅ Available' if FILTERING_AVAILABLE else '❌ Not Available'}")
        return 0
    
    if args.process:
        print(f"🎵 Processing audio file: {args.process}")
        print(f"📊 Using model: {args.model}")
        print(f"⚙️ Using engine: {args.engine}")
        print("⚠️ CLI processing not yet implemented - use GUI for now")
        return 0
    
    if args.cli:
        print("⚠️ CLI mode not yet implemented - starting GUI")
        # Future: implement CLI mode here
